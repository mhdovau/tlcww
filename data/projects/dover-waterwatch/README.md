# Dover Waterwatch

_Auto-generated from the backup. Do not edit — regenerated each run._

Dover Waterwatch, Tasmania, Australia — collecting long-term scientific observations of Dover’s waterways. The project’s own observations, measurements, photos and other project content © 2024-2026 Dover Landcare Tasmania and contributors are licensed CC BY 4.0 (full license here: https://creativecommons.org/licenses/by/4.0/ ): credit Dover Landcare Tasmania, reuse/adapt freely. Your use must not suggest that Dover Landcare Tasmania or its contributors endorse you or your use, and this licence grants no rights to their names, logos or trademarks. Uploaded third-party resources (reports, plans, spreadsheets, etc.) are not covered — they remain their owners’ copyright; seek permission before reuse. All material (including but not limited to observations, measurements, documents and photos) is provided “as is” and “as available”, without warranty of any kind, express or implied — including as to accuracy, completeness, or fitness for any purpose — and may contain errors or omissions. To the fullest extent permitted by law, Dover Landcare Tasmania and its contributors accept no liability for any loss or damage arising from its use or from reliance on it. Use it at your own risk.

## Overview

- **Observations:** 74
- **Locations:** 141
- **Members:** 3
- **Datasheets:** 3
- **CitSci URL:** https://citsci.org/projects/dover-waterwatch

## Spreadsheets (CSV)

- [observations.csv](observations.csv) — every observation in the project, one row per recorded field value (all datasheets combined).
- [locations.csv](locations.csv) — every monitoring site, with coordinates.
- Per-datasheet tables (a column per field) are linked below and in each datasheet's page.

Fields renamed over time are merged into one column under the current name (each datasheet's page lists what was merged); the long table also keeps the name each value was recorded under.

## Datasheets

- [Dover and Surrounds Adhoc](datasheets/dover-and-surrounds-adhoc/README.md) — 6 observation(s) · [CSV](datasheets/dover-and-surrounds-adhoc/observations.csv)
- [Dover Landcare Water Quality](datasheets/dover-landcare-water-quality/README.md) — 11 observation(s) · [CSV](datasheets/dover-landcare-water-quality/observations.csv)
- [Dover Rivulet Mouth Observations](datasheets/dover-rivulet-mouth-observations/README.md) — 57 observation(s) · [CSV](datasheets/dover-rivulet-mouth-observations/observations.csv)

## Files & resources

- [Huon Valley Water Quality Data 1996-2000](../../files/photos_and_files/019ee283-9163-7a06-94ce-5ec429d63dfe-542689af6756c8ff11d77a2e06dc6ff65d429af0.xlsx) — `542689af6756c8ff11d77a2e06dc6ff65d429af0.xlsx`
- [part_3_developing_a_monitoring_plan](../../files/photos_and_files/019ee2d3-63db-71c4-8523-7c439fa392aa-4cdc01ec98c67c8133b058cc16f2108619cccd83.pdf) — `4cdc01ec98c67c8133b058cc16f2108619cccd83.pdf`
- [Huon Waterwatch report 2022_23 Final 31July23](../../files/photos_and_files/019ee2d4-199b-7b65-94fa-5d78ad97ff33-b8c0ccc2312555af4a6f0acc7342c2d0ebf55b0d.pdf) — `b8c0ccc2312555af4a6f0acc7342c2d0ebf55b0d.pdf`
- [The short monitoring plan](../../files/photos_and_files/019ee2e0-eb37-7f4c-b650-c98e52b2d857-0daf3ae652320891a099fddf0ae61ed60d592fda.pdf) — `0daf3ae652320891a099fddf0ae61ed60d592fda.pdf`
- [Water Watch Physcio-chemical Monitoring Data Sheet](../../files/photos_and_files/019ee2e2-a917-7473-b09f-deeadb8e06d5-667edca6b99f4a6a7966454c91a3356e7dec5f72.pdf) — `667edca6b99f4a6a7966454c91a3356e7dec5f72.pdf`


---

_Licensing: project & contributor content is CC BY 4.0; uploaded third-party resources are not licensed for reuse; code is MIT. See [LICENSE.md](../../../LICENSE.md)._
